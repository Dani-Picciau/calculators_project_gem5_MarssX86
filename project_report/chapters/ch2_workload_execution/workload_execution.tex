...
root@{...}:~/marss/marss/disks# genisoimage -o trasferimento.iso matrix_mul
    ...
    Posizionati in \texttt{/root/marss/marss}, si andrà a creare una cartella \textit{"my\_configs"} all'interno della quale si andranno a inserire dei file per specificare il tipo di CPU che si vuole utilizzare in base alla configurazione che si vuole buildare:
    \begin{itemize}[leftmargin=1em, label=\raisebox{0.4ex}{\phantom{2}\rule{0.6ex}{0.6ex}}]
        \item \textbf{\textit{baseline.cfg}}: viene utilizzato per la configurazione baseline. All'interno di esso è specificata una CPU di tipo out-of-order (\texttt{-machine single\_core});
        \item \textbf{\textit{atom\_core.cfg}}: viene utilizzato per la configurazione CPU efficency test. All'interno di esso è specificata una CPU in-order (\texttt{-machine atom\_core});
    ...
    Ciò significa che se si volessero utilizzare delle Cache più piccole, è necesario ricompilare il simulatore prima dell'avvio di QEMU.\\
